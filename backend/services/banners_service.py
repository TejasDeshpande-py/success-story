...
def delete_s3_url(url: str) -> None:
    ...
    try:
        ...
        logger.warning("Failed to delete S3 object: %s", exc)


def get_banners(db: Session) -> list:
    """Get all banners ordered by slot."""
...
def upsert_banner(slot: int, image_url: str, db: Session, current_user: Employee) -> dict:
    """Insert or update banner at slot."""
    if slot < 1 or slot > 5:
    ...
    try:
        ...
        logger.error("Failed to save banner: %s", exc)
        db.rollback()
...
def delete_banner(slot: int, db: Session) -> dict:
    """Delete banner at slot."""
    existing = db.query(BannerImage).filter(BannerImage.slot == slot).first()
    if not existing:
    ...
    try:
        ...
        logger.error("Failed to delete banner: %s", exc)
        db.rollback()
